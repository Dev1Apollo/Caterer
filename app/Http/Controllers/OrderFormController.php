...
class OrderFormController extends Controller
{
    ...
    {
        try {
            // $itemCategories = ItemSubCategory::with(['items' => function ($query) use ($id) {
            //     $query->leftjoin('orderformmaster', 'orderformmaster.iCustomerId', '=', 'iCustomerId')->leftJoin('orderformdetails', 'orderformdetails.iItemMasterId', '=', 'itemmaster.iItemMasterId')->where('orderformmaster.iCustomerId', $id);
            // }])
            //     ->whereIn('itemsubcategory.iItemCategoryId', [2, 3])->get()->toArray();
            $itemCategories = ItemSubCategory::with('items')
                ->whereIn('itemsubcategory.iItemCategoryId', [2, 3])->get()->toArray();
            //dd($itemCategories);
            $OrderMaster = OrderMaster::where("iCustomerId", $id)->first();
            return view('orderform.add', compact('id', 'itemCategories', 'OrderMaster'));
        } catch (\Throwable $th) {
            // Rollback and return with Error
            DB::rollBack();
            return redirect()->back()->withInput()->with('error', $th->getMessage());
    ...
    public function store(Request $request)
    {
        try {
            ...
            OrderDetails::where("iOrderFormMasterId", $OrderMaster)->delete();
            foreach ($request->iItemMasterId as $iItemMasterId) {
                $iItemCategoryId = "iItemCategoryId_" . $iItemMasterId;
                $iItemSubCategoryId = "iItemSubCategoryId_" . $iItemMasterId;
                $strKG = "strKG_" . $iItemMasterId;
                $strGram = "strGram_" . $iItemMasterId;
                $strCount = "strCount_" . $iItemMasterId;
                if ($request->$strKG != "" || $request->$strGram != "" || $request->$strCount != "") {
                    $Itemdata = array(
                        'iOrderFormMasterId' => $OrderMaster ?? 0,
                        'iItemCategoryId' => $request->$iItemCategoryId ?? 0,
                        'iItemSubCategoryId' => $request->$iItemSubCategoryId ?? 0,
                        'iItemMasterId' => $iItemMasterId ?? 0,
                        'strCount' => $request->$strCount,
                        'strKG' => $request->$strKG,
                        'strGram' => $request->$strGram,
                        'iCustomerId' => $request->iCustomerId ?? 0,
                    ...
                    $Data = OrderDetails::create($Itemdata);
                }
            }

            if ($Data) {
                return redirect()->route('orderform.index', $request->iCustomerId)->with('success', 'Order Form Created Successfully.');
            } else {
                return redirect()->route('orderform.index', $request->iCustomerId)->with('error', 'Invalid request.');
            }
        } catch (\Throwable $th) {
            // Rollback and return with Error
            DB::rollBack();
